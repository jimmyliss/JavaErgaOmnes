...
public class ThumbnailHelper {
	 ...
	 * @throws IOException
	 */
	public static void gerarProprocional(InputStream in, OutputStream out, int thumbWidth, int thumbHeight, int quality, boolean pb) throws IOException {
	 ...
	 * @throws IOException
	 */
	public static void gerarTamanhoExato(InputStream in, OutputStream out, int thumbWidth, int thumbHeight, int quality, boolean pb) throws IOException {
		generateThumbRect(ImageIO.read(in), out, thumbWidth, thumbHeight, quality, pb);		
	}	
	
	private static void generateThumb(Image image, OutputStream out, int thumbWidth, int thumbHeight, int quality, boolean pb) throws IOException {
	    Graphics2DImage g2dImage = generateThumbG2D(image, thumbWidth, thumbHeight, pb);
	    saveToOutput(g2dImage.getImage(), out, quality);
	}
	
	private static Graphics2DImage generateThumbG2D(Image image, int thumbWidth, int thumbHeight, boolean pb) throws IOException {
		...
		if (thumbHeight == -1) { // Gera thumb quadrado
	    ...
	    if(x > 0 || y > 0) {
	    	...
	    	graphics2D.drawImage(tmpImage.getScaledInstance(thumbWidth, thumbHeight,
	    	          Image.SCALE_SMOOTH), 0, 0, null);	    	
	    }else{
	    	graphics2D.drawImage(image.getScaledInstance(thumbWidth, thumbHeight,
	...
	/**
	 * Gera um thumb da imagem com tamanho exato ao passado por parâmetro. <br>
	 * Redimensiona proporcionalmente a imagem para o tamanho desejado 
	 * @throws IOException
	 */
	private static Graphics2DImage generateThumbRectG2D(Image image, int thumbWidth, int thumbHeight, boolean pb) throws IOException {
		int imageWidth = image.getWidth(null);
		int imageHeight = image.getHeight(null);
	...
	private static void generateThumbRect(Image image, OutputStream out, int thumbWidth, int thumbHeight, int quality, boolean pb) throws IOException {
	...
	private static void saveToOutput(BufferedImage image, OutputStream out, int quality) throws IOException {
		...
		if(image.getTransparency() == Transparency.TRANSLUCENT) {//Converte para png
			ImageIO.write(image, "PNG", out);
		}else{//Converte para jpg
	...
	/**
	 * Classe Utilitária com um image e seu respectivo graphics2D
	 * 
	 * @author Robson J. P.
	 *
	 */
